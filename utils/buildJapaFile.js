 ...
 */
const tsRegisterContent = "require('@appruut/require-ts/build/register')\n\n";
...
module.exports = function (existingContent, ts) {
  ...
  recast.types.visit(ast, {
    visitCallExpression({ node }) {
      ...
      if (getRequireName(node) === "@appruut/require-ts/build/register") {
        usingRequireTs = true;
      }
  ...
  if (!usingJapa) {
    if (!usingRequireTs) {
      ast.program.body = recast
        .parse(japaFileContents)
        .program.body.concat(ast.program.body);
    } else {
      ast.program.body = ast.program.body.concat(
