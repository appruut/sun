...
function task() {
  ...
  const commitFile = lines(".husky/commit-msg")
    ...
    .add(
      "HUSKY_GIT_PARAMS=$1 node ./node_modules/@appruut/node-preset/validate-commit/conventional/validate.js"
    );
  ...
  pkgFile.unset(
    ...
    "node ./node_modules/@appruut/node-preset/validateCommit/conventional/validate.js"
  );

  /**
   * Copy commit convention template
   */
  template(prTemplate, join(__dirname, "conventional", "template.md"))
    .apply({})
    .save();
}

task.description = "Enforce commit message convention";
module.exports = task;
